    ...
    $(document).ready(function() {
        ...
        function renderData(res) {
            ...
            if (data.length === 0) {
                ...
                updateOverview(0, 0, 0, 0, 0, totalExpenses);
                $('#btnSettleAll').prop('disabled', true).removeClass('btn-success').addClass('btn-secondary');
                return;
            }

            // ตรวจสอบว่ายังมีเจ้าของสินค้าที่ยังไม่ได้ปิดยอดและมียอดขายหรือเบิกสะสมอยู่หรือไม่
            let hasUnsettledActivity = data.some(function(item) {
                return !item.is_settled && (item.total_sales > 0 || item.total_withdrawn > 0);
            });
