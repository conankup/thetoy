    ...
    $(document).ready(function() {
        ...
        function renderData(res) {
            ...
            if (data.length === 0) {
                ...
                thetoyTbody.append('<tr><td colspan="9" class="text-center text-muted py-4">ไม่พบข้อมูลเจ้าของร้านในฐานข้อมูล</td></tr>');
                updateOverview(0, 0, 0, 0, 0, totalExpenses, 0);
            ...
            data.forEach(function(item) {
                ...
                if (item.owner_name.toLowerCase() === 'thetoy') {
                    ...
                    const thetoyNetProfit = sumExternalGp + item.net_sales - totalExpenses;
            ...
            if (!foundTheToy) {
                thetoyTbody.append('<tr><td colspan="9" class="text-center text-muted py-4">ไม่พบข้อมูลบัญชีเจ้าของร้าน (TheToy) ในเดือนนี้</td></tr>');
            }
            ...
            // อัปเดตการ์ดภาพรวม
            updateOverview(totalSales, totalGp, totalNetSales, totalWithdrawn, totalPayable, totalExpenses, thetoyNetSales);
        ...
        function updateOverview(sales, gp, net, withdrawn, payable, expenses, thetoyNetSales) {
            const formatNum = (val) => '฿' + new Intl.NumberFormat('th-TH', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(val);
            const netProfit = gp + thetoyNetSales - expenses;
            ...
            $('#sumNetProfitDesc').text(`GP: ${formatNum(gp)} | รายได้ TheToy: ${formatNum(thetoyNetSales)} | ค่าใช้จ่าย: ${formatNum(expenses)}`);
