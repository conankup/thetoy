...
");
$stmtExpenses->execute([':start' => $start_month, ':end' => $end_month]);
$total_expenses = floatval($stmtExpenses->fetchColumn());

// --- คำนวณยอด GP รวมจากทุกคน ---
$total_gp_everyone = 0;
foreach ($ownerSummaries as $os) {
    $total_gp_everyone += floatval($os['total_sales']) * (floatval($os['gp_rate']) / 100);
}
                ...
                <div class="content">
                    ...
                    <div class="row mb-4">
                        <?php foreach ($ownerSummaries as $idx => $os): 
                            $is_thetoy = (strtolower($os['name']) === 'thetoy');
                            $raw_net_after_gp = $os['total_sales'] * (1 - ($os['gp_rate'] / 100));
                            $net_after_gp = $raw_net_after_gp;
                            if ($is_thetoy) {
                                $net_after_gp -= $total_expenses;
                            }
                            ...
                            <div class="card dashboard-card text-white shadow-sm" style="background: <?= $gradient ?>;">
                                <div class="card-body">
                                    ...
                                    <div class="d-flex justify-content-between mb-1 opacity-8">
                                        <span>รายได้สุทธิ<?= $is_thetoy ? ' (หักค่าใช้จ่าย)' : '' ?>:</span>
                                        <span>฿<?= number_format($net_after_gp, 2) ?></span>
                                    </div>
                                    <?php if ($is_thetoy): ?>
                                    <div class="d-flex justify-content-between mb-1 opacity-7" style="font-size: 0.85rem;">
                                        <span>รายได้สุทธิก่อนหัก:</span>
                                        <span>฿<?= number_format($raw_net_after_gp, 2) ?></span>
                                    </div>
                                    <div class="d-flex justify-content-between mb-1 opacity-7" style="font-size: 0.85rem;">
                                    ...
                                    <div class="d-flex justify-content-between align-items-center">
                                        <span style="font-weight: 600;"><?= $is_thetoy ? 'ปันผลสุทธิคงเหลือ:' : 'คงเหลือเบิกได้:' ?></span>
                        ...
                        <!-- ===== SUMMARY CARD FOR TOTAL GP ===== -->
                        <div class="col-xl-4 col-sm-6 mb-4">
                            <div class="card dashboard-card text-white shadow-sm" style="background: linear-gradient(135deg, #11998e, #38ef7d);">
                                <div class="card-body">
                                    <h4 class="text-white mb-3" style="font-weight: 700;"><i class="mdi mdi-percent"></i> ยอดรวม GP (ทุกคน)</h4>
                                    <div class="d-flex justify-content-between mb-1 opacity-8">
                                        <span>ส่วนแบ่ง GP สะสม:</span>
                                        <span>฿<?= number_format($total_gp_everyone, 2) ?></span>
                                    </div>
                                    <div class="d-flex justify-content-between mb-2 opacity-8">
                                        <span>ประเภท:</span>
                                        <span>รายได้ร้านค้า</span>
                                    </div>
                                    <hr style="border-top: 1px solid rgba(255,255,255,0.2);">
                                    <div class="d-flex justify-content-between align-items-center">
                                        <span style="font-weight: 600;">ยอดรวม GP สุทธิ:</span>
                                        <h2 class="text-white mb-0">฿<?= number_format($total_gp_everyone, 2) ?></h2>
                                    </div>
                                    <i class="mdi mdi-chart-areaspline card-icon"></i>
                                </div>
                            </div>
                        </div>
                    </div>
